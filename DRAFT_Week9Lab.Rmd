...
(all instruments are one 1-sec interval currently...)
...
Let's drop data missing the GPS variables for convenience, but is this the best thing to
...
on the `tsibble`, `feasts`, and `slider` package functions.
...
the data, so you'll then have a choice about it and how to fill that data in.
`tidyr::fill()` can help with this task, and the choice may be more or less
important depending on the timescale you're working on. Given the small amount
of missing and the one second timescale, it's not unreasonable to introduce a
few rows with NAs here.
...
years, etc. This is a "block average". For the second average
...
map_times <- ts_data %>% 
  ...
  # make a map_time variable, dropping the seconds (the `$` indicates the
  # character pattern should be at the end of the string)
...
<!-- IS THIS PLUME DETECTION OR BACKGROUND CHARACTERIZATION?? -->
