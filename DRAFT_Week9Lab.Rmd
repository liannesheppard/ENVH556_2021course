...
data. Instruments included in the lab include a P-Trak particle counting
...
gps <- read_csv(file.path(data_dir, gps_file)) %>%
   ...
   # set column names
   setNames(c("date", "time", "latitude", "longitude", "altitude", "speed")) %>% 
...
ptrak_noscreen <-  read_tsv(file.path(data_dir, ptrak_noscreen_file), skip = 29) %>% 
   ...
   # name columns
   setNames(c("date","time", "ptrak_noscreen_conc")) %>% 
...
ptrak_screen <-  read_tsv(file.path(data_dir, ptrak_screen_file), skip = 29) %>% 
   ...
   # name columns
   setNames(c("date","time", "ptrak_screen_conc")) %>% 
...
all_data <- reduce(instrument_list, full_join) %>% 
   ...
   # ensure data are arranged in datetime order
   arrange(datetime)
...
Let's make a quick visualization of our data to see what we're working with.
...
# we'll use our instrument data list
lapply(instrument_list, function(i){
   ...
   # use list names
   setNames(names(instrument_list))

```

It appears that some of our missingness is due to instrument reporting. Some
measurements did not occur at 1-second intervals. From this we can also see
there are duplicate measurements for each instruments (difference of zero
seconds).
...
Let's drop data missing the GPS for convenience, but is this the best thing to
under all circumstances and for all variables?
...
We've focused most of our effort on `tidyverse` tools this term, so let's focus
on the `tsibble`, `feasts`, `slider`, and `fable` package functions.
...
needs to be unique and complete. Adding these new time rows introduces NAs to
the data, so you'll then have a choice abot it and how to fill that data in.
`tidyr::fill()` can help with this task, and the choice may be more or less more
or less important depending on the timescale you're working on. Given the small
amount of missing and for the one second timescale, it's not unreasonable to
introduce a few rows with NAs here.
...
autocorrelation plot helps identify at what lag (or time interval) data are
less correlated.
...
concentrations are less correlated. So let's convert our 1-second data to the
...
years, etc. This equates to a "block average". For the second average
...
Let's inspect their timeseries:
...
a little "extra" space to the plotting area and outputs an object used to define
the extent of the mapping region. You might find it helpful in the future. It
requires `stringr`and `tidyr`.
...
(I've split up getting the map objects from mapping so you can run the mapping
chunk interactively without re-downloading the stamen map.)
...
# make a map of the base layer of stamen tiles (without messages)
...
We're focusing on time-varying data in this lab, so to help our visualization,
let's collect the times of some of our measurements.
...
Taking the ratio of pollutants...
...
Another strategy to aid in unmixing space-time processes in mobile monitoring
data is to remove the "background" atmospheric processes.

## Remove a Percentile of the Data

One method to do that is to subtract a percentile of the data 

```{r remove.moving.percentile}
#-----remove moving percentile-----

# specify the time in minutes from which to calculate the percentile
time_min <- 10

# specify the percentile to remove
perc <- 0.10

# we can use use `slider` to calculate the percentile value to remove
ts_data <- ts_data %>% 
  mutate(perc_remove = slide_dbl(co2_conc, 
                                 ~quantile(.x, probs = perc, na.rm = TRUE), 
                                 .before = (time_min*60)/2, 
                                 .after = (time_min*60)/2), 
         co2_adj_perc = co2_conc - perc_remove) 
...
Another way to remove trends in tmeseries data is to fit a smoother to the data and subtract the fitted values from the observations. 
...
# add predictions and residuals to ts_data
ts_data <- ts_data %>% 
...
# we're going to cheat again and plot the series seperately instead of from a
# combined dataframe

ggplot(ts_data) + 
  
  # plot the original data
  #geom_line(aes(x = datetime, y = co2_conc), color = "darkgray") +
  
  # plot the percentile-adjusted data
  geom_line(aes(x = datetime, y = co2_adj_perc), color = "blue") +
  
  # plot the loess-adjusted data
  geom_line(aes(x = datetime, y = co2_adj_loess), color = "red") + 
  
  geom_hline(yintercept = 0, color = "black") +
  labs(x = "Time", y = "Residual CO2 (ppm)") +
  theme_bw()
