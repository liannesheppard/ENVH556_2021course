...
[Lianne, we've recommended inline code, use here? drafted for each if yes]
    - `REC`: 688           `r nrow(REC)`
    - `cohist`: 6,040      `r nrow(cohist)`
    - `jem`: 6,480         `r nrow(jem)`
...
#[Lianne, maybe we should be consistent with "_uniq" and "_u"? do they mean the
#same thing, unique?]
cohist_uniq <- cohist %>%
    group_by(year) %>%
    summarise(period_u = first(period), 
              .groups = "drop")

# join jem and cohist as an inner join (only keep those that match) and then
...
# make period a factor, then recode to create periodfac variable that aligns
# across datasets
cohist <- cohist %>% 
...
[Lianne, I looked at the co_ratio by year and by quarter - there are differences, but they didn't seem wild to me. (I'll send along figure). We can re-visit this together if you'd like.]

TODO:  At a minimum it would be worthwhile to look at how these compare with the
numbers given in Table 3 of DEMS IV.

[Lianne, I started looking at this below, but not sure if that is what you were
getting at.

Currently, we estimate CO with 2 different "simple_fit" models (one by year, one
by period) and merge dataframes, creating the `JEM` object. The second time we
do that we reassign all the same variables, overwriting them, including `JEM`.
One thought is to assign `JEM_year` and `JEM_period` so we have both objects at
the end of the script (to compare?). OR name `co_AM_year` and `co_AM_period`
(and similar for `co_ratio` and `rec_mjy`) variables which we could then merge
the JEM objects together into one.]
-->
...
# these can be merged if desired...
