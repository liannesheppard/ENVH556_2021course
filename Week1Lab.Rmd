...
Note:  The following three chunks should typically appear at the beginning of
...
# Ordinarily we set include=FALSE for this chunk; just showing it for the first 
# lab.
...
included in [tidyverse](https://www.tidyverse.org).  Our goal in this class is
to prioritize these tools as much as possible while recognizing that there are
many ways to accomplish the same task and we don't want to get too bogged down
in the best way to accomplish our goals.
...
described in Coble et al 2010.  We are focusing on the personal data collected
...
paths set up.
    + First assign your current path to a variable name (see *getwd* chunk).  
    + Then tell R where your data reside (see *datasets.path* chunk).  The 
      `dir.create()` command will make a directory if it doesn't exist.  The 
      `file.path()` command allows you to refer easily to paths across operating 
      systems.
    + See the following chunks to accomplish this:
```{r getwd}
#-----getwd--------
ProjectPath <- getwd()
#ProjectPath
...
* Read in the data 
     ...
     defined by `datapath`.
```{r read.data, eval=TRUE, error=TRUE}
#-----read.data--------
# Note:  for simplicity of typing in this lab we're calling the dataset DEMS,
...
As an alternative, you can also read the data file from a web site:

```{r read.data.from.a.web.site, eval=TRUE}
...
+ `(DEMSt <- as_tibble(DEMS))`  create a tibble version and look at DEMS in the 
  tibble format
+ `select(DEMSt, facilityno, ecdata, nodata, no2data)` allows you to zoom in on 
...
# Now for a tidyverse option.  We will also save the version in case we want to
# use tidverse commands.

# Create and print the tibble DEMSt (The parentheses surrounding the command
# tell R to print the result of the assignment.)
(DEMSt <- as_tibble(DEMS)) 

# Our application of select zooms in to focus on the key exposure data
select(DEMSt, facilityno, u_s, ecdata, nodata, no2data) 
...
# a different basic summary of each variable
describe(DEMS) 
...
(@) `ggplot2` (part of `tidyverse`)  has great tools for plotting.  First we
    ...
    (a) See chunk comments and the notes following the chunk for option
        suggestions.
    
    (b) `ggplot` gives warnings about omitted data and messages about better
...
# Plot 2 with density base plot, just the histogram, now named 'p' for ease of
# re-use:
...
Typically exposure data appear to be log-normally distributed.  This section we
transform and plot the transformed data.
...
In particular, Chapter 1 gives a nice introductory overview and Chapter 22
focuses on the details you need to facilitate good communication.
...
# Note1:  the scales="free" option allows a different scale for each plot.  In
# this case it makes the individual plots more informative. 
# Note2:  the smoother was failing for facilty E for unknown reasons; without
# scales="free" all the plots had huge ranges
ggplot(data = DEMS, aes(na.rm = TRUE)) +
...
the results in Figure 2 of Coble et al (2010).  (We will show you how to
transform data in R next week.)
...
on Canvas for the format and content of your lab report.  You may also be
interested in some of the tips on the Canvas Wiki page: *Helpful Hints on Lab
Report Write-ups*. <!-- TODO:  Add hyperlinks -->
...
(1) Make table(s) summarizing four exposure variables in this dataset:  Nitrogen
    ...
    (b) Getting GM and/or GSD in a table in R:  This lab doesn't show ow to get
        the GM and GSD using `tidyverse`; it should be possible using `dplyr`,
        perhaps with `mutate`.  You can use the `EnvStats` package with `tapply` and
        `cbind`.  To get the AM, ASD, GM, GSD, N in order:
...
The following approaches to making tables are available, though not recommended.
These include a. the `ddply()` function in the `plyr` package, b.
`summarizeBy()` from the `doBy` package, and c. `aggregate()` which is part of
base R but more difficult to use.

(a) with `ddply()`:  Out of date so code hidden  

```{r table.with.ddply, echo=FALSE, eval=FALSE}
#------------------table.with.ddply------------------
...
```{r table.with.summaryBy, echo=FALSE, eval=FALSE}
...
(c) with `aggregate()`:  
    The following gives means of 3 variables.  Note:  by comparing the results 
    from facility H6, we can see that the the missing data were removed 
    separately by each variable.
...
aggregate(DEMS[c("ecdata", "no2data", "nodata")],
          list(facility = DEMS$facilityno), mean, na.rm = TRUE)

```

### A1.2 Base R option for a histogram of ecdata with a density curve added to the histogram {-}
...
### A1.3 Logarithmic transformations in base R: {-}
...
### A1.4 Base R option for a basic scatterplot and overlaid regression line {-}
...
The next two chunks should be included in every R Markdown appendix so that you
document your code and session information.  This supports the reproducibility
of your work.
