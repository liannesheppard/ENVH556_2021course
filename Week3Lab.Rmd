...
The purpose of this lab is to get practice using and reporting regression models with a focus on regression for association and some beginning work on regression for prediction.   We will use the ACT snapshot data for this lab.
...
snap2 %>% 
  dplyr::select(ln_nox, preds_sum, resids_sum, preds_fall, resids_fall, 
                preds_win, resids_win) %>% 
...
4.	Replicate the season-specific models in Table 4 of [Mercer et al, 2011](https://doi.org/10.1016/j.atmosenv.2011.05.043).  (Only focus on the LUR model results.  We’ll talk about cross-validation in an upcoming week so recognize your R^2^ should be a bit bigger and your RMSE should be a bit smaller than the values in the paper.)  
