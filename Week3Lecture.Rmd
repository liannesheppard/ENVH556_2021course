...
pacman::p_load(dplyr, ggplot2, knitr, multcomp, dplyr, modelr, car)  
```
...
# # dplyr option (consistent, but maybe not better here?)
# snapshot %>% 
#     group_by(seasonfac) %>% 
...
# # dplyr option 
# # (Lianne - this is a lot more code, but I didn't see how you were using the 
# # output in the slides -  e.g. getting coefficients, etc. feel free to remove. 
# # If keeping, add `broom` to package loading)
...
```{r slide25.season.adjusted}
#-----slide25 season adjusted----
...
##[CZ: Lianne, I added season rownames and coefficient names to the matrix, K, but wasn't able to figure out the matrix definition from the names either.]
...
#[CZ: Lianne, are "cluster", "lat_m" and "long_m" supposed to be omitted from the regression model? If so, should they be omitted from the selected variables? The comment says "only variables of interest" so just trying to limit any confusion. In the chunk below,  I broke these steps up a little bit to be more readable/flexible, but feel free to keep either, in this case it probably isn't so important.]

# First create a summer only dataset with only the variables of interest
summer <- snapshot %>%
    dplyr::filter(season == 1) %>%
    dplyr::select(ln_nox, D2A1, A1_50, A23_400, Pop_5000, D2C, Int_3000, D2Comm, 
                  cluster, lat_m, long_m)

# Then run the regression
summer_mod <- lm(ln_nox ~ D2A1 + A1_50 + A23_400 + Pop_5000 + D2C + Int_3000 + D2Comm, 
                 data = summer)
...
#[CZ: is there a reason to create a second dataframe "2" when adding predictions & residuals? It is done above too with the snapshot & snapshot2 dataframes]
