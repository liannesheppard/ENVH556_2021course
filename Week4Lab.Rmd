...
pacman::p_load(knitr, dplyr, tidyr, ggplot2, modelr, stringr)
...
# need to be pre-assigned (so that can be deleted if this version is kept, {but 
# keep n_covars object assignment})]
...
################################################################################
# CHRIS:  I don't know how to set the limits on this combined plot.  Do you?
# Lianne, the strategy below is certainly not ideal, but the appearance is the 
# same as if we were able to restrict the axis limits. An alternative would be 
# to combine the separate plots with `egg::ggarrange()` or similar (noted 
# below).
################################################################################
...
# [Lianne, if you have a programmatic idea we calculate a number]
y_lim <- 5
...
temp <- res %>% 
  ...
  # seperate the "Source" column for in and out of sample
  separate(col = Source_Estimate, into = c("Source", "Estimate") ) %>% 
  
  # create a new column with the "plot values", setting high values to NA
  mutate(plot_value = ifelse(Estimate == "RMSE" & value > y_lim, NA, value))

# plot both model-based (in sample) and CV generated MSE and R2
combined_plot <- ggplot(temp) +
  geom_point(aes(x = n_pred, y = plot_value, color = Source)) + 
  geom_line(aes(x = n_pred, y = plot_value, color = Source)) +
  xlab("Model Complexity (# of terms)") +
  ylab("") +
  scale_x_continuous(breaks = c(seq(0, 63, 5))) +
