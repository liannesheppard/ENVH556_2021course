...
# # Lianne, here is an option to get things in one pipeline (still not great though)
# DEMS %>%
#     # summary by facility
#     group_by(facilityid) %>%
#     summarise(N = n(), 
...
# # Lianne, a bit easier way to do this below (perhaps less straightforward though?)
# fvc <- snapshot %>%
#     as_tibble() %>%
#     filter(season == 2)  %>%
#     group_by(group_loc) %>%
...
# # Lianne, one pipeline option below like earlier (though `vc_summary` is used later on)
# fvc %>%
#     group_by(group_loc) %>%
#     summarise(N = n(),
#               mean = mean(ln_nox),
