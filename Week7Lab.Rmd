...
# create the pure measurement error dataframe (setting the seed if desired)
pure_data <- me_pure_data(n_subj = 10000, seed = 556)
...
result_pure_sp <- lapply(seq_len(n_iter), function(x) me_pure(pure_data)) %>% 
...
temp <- pure_data %>% 
  ...
  pivot_longer(cols = c(contains("Berk_"), contains("class_")), 
               names_to = "error_type", 
               values_to = "error_outcome")

# make plot
ggplot(data = temp, aes(x = y, y = error_outcome)) + 
