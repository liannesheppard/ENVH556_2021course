...
pure_data <- me_pure_data(n_subj = 250)
...
Lianne, here is an alternative to the chunk above. I thought about what I was
doing wrong and the the solution to get one legend symbol is to make both of the
lines the same type (before one was an abline and one was a geom_smooth). ggplot
saw those the same way it would see a geom_point() and geom_line(), putting both
symbols in the legend. The difference with the version below is that the dashed
line does not span the whole plot area as ablines do, but just the range of the
data. So you'll just have to choose what your preference/priority is.

With the new type of line, depending on how many points you'd like to plot, I'd
adjust the dashed line color since it can be hard to see. If you'd like 10000
points "gray" seems fine, with 250 points, "black" might be better?

If you want to keep the chunk below just delete "ALTERNATE" in the chunk header
and the whole chunk above.

-->

```{r plot.pure.ME.data.ALTERNATE, message=FALSE}
#-----plot pure ME data ALTERNATE-----
...
ggplot(data = temp, aes(x = error_exposure, y = y)) + 
  ...
  # best fit line given true exposure x
  geom_smooth(method = lm, se = FALSE, data = temp, 
              aes(x = x, y = y, linetype = "dashed", color = "gray") ) +
