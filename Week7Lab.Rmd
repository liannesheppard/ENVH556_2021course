...
for this exercise is `mesim_pure` found in the file named *MEprograms_pure.R*.
Some sample code for running this program is below.

<!--[Lianne, this file is named "mesim_pure.R" not "MEprograms_pure.R" as above, correct?] -->
...
3.  We use **simulation** to get the statistical properties of our results:  To
    simulate this we replicate steps 1. and 2. `n_iter` number of times.  Our
    default value for `n_iter` is 1,000.  For testing it is good practice to use 
    a MUCH smaller number of replicates, such as `n_iter=10`.

4.  **Summarize the results** of the exposures and health effect estimates after
    the program completes

```{r measurement.error.simulation_pure}
#-----measurement error simulation for pure Berkson & classical-----

#[Lianne, I've edited this section heavily, as well as the .R source file. I
# believe they are easier to follow now without the nested list structure.
# There was a difference between the single and multi-core versions before my
# edits, and I believe it was due to the seed argument in the multicore version.
# If that is set to FALSE, the first replicates match, but then differ slightly
# in other replicates. The documentation isn't clear to me, but mclapply
# continues to do something with the seed in subsequent iterations.]
...
# Step II:  Iterate
result_pure_sp <- lapply(1:n_iter, function(x) me_pure()) %>% 
...
#   
#   # Step II:  Iterate
#   result_pure_mp <- mclapply(1:n_iter, function(x) me_pure(),
#                              mc.cores = 8,
#                              mc.set.seed = TRUE,
#                              mc.preschedule = TRUE) %>% 
#     
...
# Display results 
# TODO: remove N from the table; how to put it in the caption [Lianne, is
# something like this what you had in mind? if so, you can remove the newline
# break if desired and change digits. Alternatively, replace "unique(res$N)"
# with "n_iter"]
kable(res_pure %>% select(-N), 
      digits = 3,
      caption = paste0("Health effect estimate properties. ", 
                       "\nN = ", unique(res_pure$N), " iterations.") 
      )
...
#[Lianne, I've edited this section heavily too, as well as the .R source file. Check to make sure it looks OK, if not, we can always go back to the old files.]
...
# Step II:  Iterate
result_like_sp <- lapply(1:n_iter, function(x) me_like()) %>% 
...
# Display results
# [Lianne, I added the same sort of modifications here as above if you want to keep]
kable(res_like %>% select(-N), 
      digits = 3,
      caption = paste0("Health effect estimate properties. ", 
                       "\nN = ", unique(res_like$N), " iterations.") 
      )
