...
# downloader
pacman::p_load(tidyverse, knitr, dplyr, gstat, maps, ggmap, stars,
...
Note that geom_sf plots in lat-long units by default, even though our data is stored in a different projection. (With the projection defined, it would be trivial to convert our points to a different projection if we wanted as well). We can check that map projections are working properly and explore the study area on [Google Maps](https://www.google.com/maps?q=50.97,5.74)
...
# Define a wrapper function krige.cv.sf that does not drop the projection
# of the sf object. This repairs an apparent bug in krige.cv()
krige.cv.sf <- function(formula, locations, model = NULL, ..., beta = NULL, 
  ...
  krige.cv1 <- krige.cv(formula, locations, model = NULL, ..., beta = NULL, 
	nmax = Inf, nmin = 0, maxdist = Inf, nfold = nrow(locations), 
	verbose = interactive(), debug.level = 0)
...
bubble.krige.cv.sf <- function(cv.out, plot_title){
  ggplot(data=cv.out) +
...
meuse.CV5 <- krige.cv.sf(log(zinc)~1, meuse, model = v.fit, nmax = 40, nfold=5)
# plot the residuals
bubble.krige.cv.sf(meuse.CV5,"log(zinc): 5-fold CV residuals")

# ordinary kriging, LOOCV
meuse.CVLOO <- krige.cv.sf(log(zinc)~1, meuse, model = v.fit, nmax = 40)
# plot the residuals
bubble.krige.cv.sf(meuse.CVLOO, "log(zinc): LOO CV residuals")

# universal kriging, 5-fold cross-validation 
meuse.CV5uk <-
  krige.cv.sf( log(zinc) ~ sqrt(dist), meuse,
    model = m.uk, nmax = 40, nfold = 5 )
# plot the residuals
bubble.krige.cv.sf(meuse.CV5uk,"log(zinc) UK on sqrt(dist): 5-fold CV residuals")

# universal kriging, LOOCV
meuse.CVLOOuk <- krige.cv.sf(log(zinc)~sqrt(dist), meuse, 
                        model = m.uk, nmax = 40)
# plot the residuals
bubble.krige.cv.sf(meuse.CVLOOuk,"log(zinc) UK on sqrt(dist): LOO CV residuals")
...
```
Here is code to project into Lambert coordinates in R:
```
...
# [Lianne, here is an option, not sure it is better. The ln_nox really scruntches up the scale]
  # (Leaving this code from Chris in here for now, fairly similar to above -- DLS)
  #ggplot(data=fall) + 
...
# helper function for bbox (you can modify and use this for your own work)
# DLS note--leaving this in here for now, but maybe not needed, or maybe could
# adapt the above code into bbox function instead.
...
#   
#   # drop rows with NA (first converting to a dataframe if not already)
#   temp <- .data %>% as_tibble() %>% drop_na(all_of(c(long, lat)))
#   
...
g + geom_point(data=new_grid,aes(x = st_coordinates(new_grid)[,'X'],
  ...
  my_theme
# 
# if (!file.exists('Datasets/ne_10m_land.zip')){
#   download.file('https://www.naturalearthdata.com/http//www.naturalearthdata.com/download/10m/physical/ne_10m_land.zip',
#                 'Datasets/ne_10m_land.zip')
# }
# if (!file.exists('Datasets/ne_10m_land/ne_10m_land.shp')){
#   unzip('Datasets/ne_10m_land.zip',exdir='Datasets/ne_10m_land')
# }
# land <- st_read('Datasets/ne_10m_land/ne_10m_land.shp')
# land <- land[land$featurecla=='Land',]
# land <- st_crop(land,st_bbox(la_grid))
# trial <- st_overlaps(la_grid,land)
