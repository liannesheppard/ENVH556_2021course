...
if (utils::packageVersion('gstat') < package_version('2.0.7')){
  pacman::p_load_gh('r-spatial/gstat', force = TRUE)
} else {
  library(gstat)
}
...
<!-- TODO:  Get the datasets to Brian's directory The reason for the changes on
3/5/19 had to do with needing lambert coordinates and maybe using a smaller grid
for the predictions -->
...
The `sf` or Simple Features package is the more modern way to reference spatial
data in R.  We use `sf` in this lab.  The alternative is `sp` and we include a
few references to it for completeness.
...
each bin is also 6,833).  This is because the default distance is 1/3 of the
maximum distance.  We can change the default distance using the `cutoff =`
option.  (See example with the snapshot data.)
...
Credit:  The following examples were modified to use the `sf` package and follow
the code [here](https://github.com/r-spatial/gstat/blob/master/demo/krige.R).
...
* `lat_m` and `long_m`, which may be in UTM, and which supposedly had been
projected to a flat surface, and
...
for this dataset because these are in meters and projected for a flat surface so
distance calculations can be done directly.  To see a documentation of the
...
# columns like lat_m, long_m, lambert_x, lambert_y, because we can easily
# reproject the data however we like later
fall <- snapshot %>%
...
# specify url a zip file to a shapefile
...
# [CZ: Lianne & Dave, this is not running for me... anyone else? remove since
# we've got a bubble plot below that works?]
#bubble(fall, "ln_nox", col="blue", main = "ln(NOx) Concentrations (ln(ppb)")

# we can also produce a bubble plot of the data with ggplot
ggplot(data=fall) +
  geom_sf(data=fall,aes(size=ln_nox), color='blue', alpha=1/2) +
  ggtitle("ln(NOx) Concentrations (ln(ppb)") +
  theme_bw()

# [CZ: Lianne, here is an option, not sure it is better. The ln_nox really
# scruntches up the scale]
  # (DLS -- leaving this code from Chris in here for now, fairly similar to
  # above -- DLS) 
  # CZ: this is no longer working for me either... was there a class change or
  # something? either way, it can probably be removed since there are 2 working
  # plots here.
  ggplot(data=fall) +
...
# For some reason, geom_sf() did not work for this (bug?), so needed to use
# geom_point and manually add the extracted XY coordinates
...
# Still true? Was it the cutoff was too short?
# [CZ: Lianne & Dave, this is working for me. you too? if so, remove "TODO"
# note above?]
...
# [CZ: Lianne & Dave, I get different numbers than in the comment above. Edit or
# remove? I also don't see a nugget anymore in the output...]
vf.fit
...
# Mercer.
# [CZ: Lianne & Dave, Again, no nugget in the output here...]
vfc.fit 
...
the original grid in order to get the longitude and latitude coordinates.  The
...
# also transform predicted NOx to the native scale
...
# [CZ: Lianne & Dave, This chunk no longer works (failing at the creation of
# `spdf_new_grid`). Do the issues this chunk is trying to address exist
# anymore?]
...
# create theme so it can easily be re-used
my_theme <- theme(
...
Now show an example plotting smooth gridded predictions on the map with tiles
...
# [CZ: Lianne & Dave, this chunk fails, but with Dave's note below, is it
# something we still need/want?]
...
# [CZ: Lianne & Dave, I looked and couldn't find updates/replacements for this
# file. Suggest removing.]
