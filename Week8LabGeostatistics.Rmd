...
Now plot the snapshot data using the same plotting code similar to that applied
...
# we show a similar color and size varying bubble plot of the data with ggplot
ggplot(data=fall) +
...
(partial sill, range, nugget).  In the following chunk we fit two sets of models
for didactic purposes only:  an OK model with no trend (i.e. no covariates in a
LUR), and a UK model with trend (i.e. the covariates in the common model).  We
plot the variogram fits to both models for comparison.  (Note:  this comparison
is for educationa/practice purposes only.  Scientifically we don't think an OK
...
second visible chunk adds our predictions to a Stamen map.  All plots show the aree restricted to the subset of the grid we created above.  The second plot shows
