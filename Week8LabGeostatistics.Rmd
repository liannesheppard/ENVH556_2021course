...
new_grid <- inner_join(as.data.frame(la_grid),
                       as.data.frame(kc_la),
                       by = c("lambert_x", "lambert_y")) %>% 
  mutate(NOx = exp(var1.pred))
  #filter(NOx != Inf)
...
# make plot
g + geom_point(aes(x = longitude, y = latitude, color = NOx),
               data = new_grid, alpha = 0.5) +
