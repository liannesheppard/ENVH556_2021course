...
# of the sf object. This repairs a bug in gstat.cv(), called internally by krige.cv()
...
fall.CVcluster.ok <- krige.cv.sf(ln_nox~1,
...
fall.CVcluster.uk <- krige.cv.sf(ln_nox ~ D2A1 + A1_50 + A23_400 + Pop_5000 + D2C + Int_3000 + D2Comm,
    st_transform(fall,lambert_proj),
    model = vfc.fit,
    nfold = fall$cluster,
    )
...
g + geom_point(data=new_grid,aes(x = st_coordinates(new_grid)[,'X'],
  ...
  my_theme
# 
# Dave getting ready to crop out the water points, but haven't bothered to implement yet.
