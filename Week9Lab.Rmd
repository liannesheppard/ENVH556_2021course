...
the data, so you'll then have a choice about if and how to fill the non-index
data in. `tidyr::fill()` can help with this task, and the choice may be more or
less important depending on the timescale you're working on, the amount of
missingness, and if there are consecutive measurements mising. Given the small
amount of missing and the one second timescale, it's not unreasonable to
...
All our example data are on the 1-second timescale; however, it is common to have multiple timescales in a suite of direct-reading instruments. Using the techniques above, you'll be able to average different time intervals to a common schedule. Once the measurements are on a shared timescale, they can be merged/joined as we have shown in the "Merge Instrument Data" section.
...
# we're going to cheat again and plot the series separately instead of from a
# combined dataframe
