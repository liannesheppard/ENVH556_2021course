...
pacman::p_load(knitr, dplyr, tidyr, readr, stringr, ggplot2, hms, zoo,
...
nm while the screened P-Trak collects particles in the range of 36-1,000 nm.
...
#refine bbox to align with study domain. Take out the approach to the study region (drive from UW and include location of the airport on the map).
...
#specify the rolling time window
window_val = 60
...
ts_data <- ts_data %>% 
  mutate(co2_adj = (co2_conc - co2_background_min),
    
        co2_lag = lead(co2_adj), 
         
         #calculate squared difference in CO2
         co2_diff_sq = (co2_adj - co2_lag)^2,
         
         #windsorize
         co2_diff_sq = replace(co2_diff_sq, co2_diff_sq> 
                                  quantile(co2_diff_sq, wp,  na.rm=T), 
                                quantile(co2_diff_sq, wp,  na.rm=T)),
        
         #calculate a 2 minute rolling mean of CO2 differences
         roll_co2_diff = rollapply(co2_diff_sq, 
                                   window_val,
                                   mean,
                                   align='center',
                                   fill=NA, 
                                   partial = window_val/2))
#specify cutoff for peak change in slope values
pqv = 0.6

ts_data <- ts_data %>% 
  mutate(
         co2_is_peak = if_else(roll_co2_diff >= quantile(roll_co2_diff, pqv, na.rm=T), TRUE, FALSE)
         ) 
...
emission sources in data with both temporal and spatial variability. We'll use the P-Trak data to explore the idea of pollutant ratios by plotting the proportion of smallest particles (20-36 nm to the total cocentration of UFP particles)
...
#display ratios that are greater than zero 
