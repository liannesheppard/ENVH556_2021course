...
autocorrelation of these data. At what time do the CO<sub>2</sub> appear less
correlated?
...
<!-- Lianne & Elena, 
CURRENTLY THIS SECTION IS WRITTEN SHOWING 2 METHODS (MOVING MINIMUM AND
PERCENTILE). IF YOU FIND ONE MORE IMPORTANT/INFORMATIVE THAN THE OTHER - KEEP
ONE AND REMOVE THE OTHER (AS WRITTEN, ONLY THE MIN VERSION IS ASSIGNED TO AN
OBJECT). WE COULD ALSO SHOW ONE AND ASK STUDENTS TO DO THE OTHER IN THEIR
HOMEWORK -->

## Rolling Percentile & Minimum
...
of 30 min time intervals. We'll use that approach in upcoming sections.

```{r calculate background.as.moving.minimum, warning=FALSE}
#-----calculate background as moving minimum-----
...
ts_data <- ts_data %>% 
  mutate(co2_background = slide_dbl(co2_conc, 
                                 ...
                                 .before = (time_min*60)/2, 
                                 .after = (time_min*60)/2), 
         co2_background = na_if(co2_background, Inf), 
         co2_adj = co2_conc - co2_background
         ) 
...
CURRENTLY CODE IS NOT RUN WITH CHUNK HEADER OPTIONS
-->

Another approach to background standardization in mobile monitoring data is to
fit a smoother to the data (and subtract the fitted values from the
observations, if analysis requires).
...
ts_data %>% 
  mutate(co2_background = predict(mdl, ts_data)) %>%
...
We'll demonstrate with the the rolling minimum approach.
...
ggplot(ts_data) + 
  ...
  # plot co2 background
  geom_line(aes(x = datetime, y = co2_background)) + 
...
ts_data <- ts_data %>% 
  mutate(co2_peak = slide_dbl(co2_adj, 
                              ~quantile(.x, probs = perc, na.rm = TRUE), 
                              .before = (time_min*60)/2, 
                              .after = (time_min*60)/2), 
         co2_is_peak = if_else(co2_adj >= co2_peak, TRUE, FALSE)
         ) 
...
<!-- Lianne & Elena, 
THIS WAS IN KOLB (REFERCED IN BRANTLEY) AND SEEMED DOABLE, USING THE IDEA OF
...
# calculate ratio
ts_data <- ts_data %>% 
  mutate(ptrak_ratio = ptrak_screen_conc / co2_adj)
