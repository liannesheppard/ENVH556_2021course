...
# show class
# Note that POSIXt is a virtual class which cannot be used directly. A virtual
# class ‘POSIXt’ exists from which both of the classes [POSIXct and POSIXlt]
...
current time and date these values are `r as.numeric(Sys.time())` seconds and `r
as.numeric(Sys.Date())` days. When printed to the console, they are displayed in
a human-readable format along the the International Organization for
...
object specifically for each instrument. There are several reasons on practice
why we need to pay careful attention to this:  
* the time scale of each instrument can be different, 
* the internal instrument clocks can be different from each other and/or the current time, or
...
Here, we'll focus on CO$_2$ data from Car 1 (there were 2 cars), for your
...
maximum number of obervations in the dataset.  To learn that use `glimpse` or
`str`.
...
common column names and joins on thse.  Alternatively you can add this argument:
`by = c(datetime, date, time)`.
...
We observe that early rows of the dataset have missing GPS and CO2 data, so we
know this is something we need to investigate further.
...
Next we try to precisely determine how much missing data we have and what are the likely sources of
missingness in our data.  First we calculate the total and percent missing for every variable in the dataset.  We observe that the presence of missing data depends upon the instrument.
...
the other instruments (there is no GPS data at the beginning of the all_data dataframe).
This may be due to the fact that the other instruments were turned on for "warm-up"
...
Let's also fill the gaps in time. In other words, we have decided in this analysis that the index column `datetime`
needs to be unique and complete. In standard time series analyses the time increment between every consecutive observation is the same.  Adding these new time rows introduces
"explicit" NAs to the data, so you'll then have a choice about if and how to
fill in the non-time data. `tidyr::fill()` can help with this task, and the
choice of how to fill may be more or less important depending on the time scale you're working
on, the amount of missingness, and if there are consecutive measurements
missing. Given the small amount of missing and the one-second time scale of the
...
autocorrelation of these data, i.e., which autocorrelations statistically different from zero. At what time do the CO<sub>2</sub> appear less
correlated?
...
the autocorrelation plot above, we can see at a lag of 60 seconds, the CO<sub>2</sub>
concentrations are no longer correlated. So let's convert our 1-second data to some
longer time scales. One package that helps with averaging tasks is `slider`.
...
years, etc. This is a "block average" where the period we are averaging over is based on an attribute of the data. For
...
(a 2-minute average) we'll demonstrate a moving ("sliding" or "rolling") average, where the
...
# we're going to use a simpler approach here and just manually specify each series rather
...
# TODO: Chris:  Here or earlier figure out how to offset the 5-minute data so they line up on the plot
...
save the map for later. If the map file has already been downloaded and saved, we can
...
you've already downloaded and saved a `map.rds` file, but then you want to modify the
...
One approach to do that is a moving percentile of the data.

```{r calculate background.as.moving.percentile}
#-----calculate background as moving percentile-----
# Note:  we don't save the updates to ts_data in this chunk, but do so in a
# later chunk and assign the new names with suffix _pct for moving percentile
...
ts_data %>% 
  mutate(co2_background = slide_dbl(co2_conc, 
                                    ~quantile(.x, probs = perc, na.rm = TRUE), 
                                    .before = (time_min*60)/2, 
                                    .after = (time_min*60)/2), 
         co2_adj = co2_conc - co2_background) %>% 
  
  ggplot(aes(x = datetime, y = co2_background)) + 
    geom_line() + 
...
perc*100`^th^ percentile of the data to calculate the background CO<sub>2</sub>
concentration.

As we talked about in class, another approach is to calculate the moving minimum
of 30-minute time intervals. We'll use that approach in the next section.  Also let's compare our two background estimation approaches in one plot:

```{r plot moving minimum, warning=FALSE}
#---- plot moving minimum ----
# calculate and plot the moving minimum; add it to ts_data along with the moving
# percentile

# specify the time in minutes from which to calculate the minimum
time_min <- 30

# this repeats the moving percentile from above and adds the moving minimum
# we can use use `slider` to calculate the minimum equal to background. 
# `min()` will return Inf when all values are NA; use `na_if()` to avoid.
ts_data <- ts_data %>% 
  mutate(co2_background_pct = slide_dbl(co2_conc, 
                                    ~quantile(.x, probs = perc, na.rm = TRUE), 
                                    .before = (time_min*60)/2, 
                                    .after = (time_min*60)/2), 
...
# plot the two background estimates on the same plot
  ggplot(data = ts_data) + 
    geom_line(aes(x = datetime, y = co2_background_pct), lty = 1, color = "black") + 
    geom_line(aes(x = datetime, y = co2_background_min), lty = 1, color = "red") + 
    labs(x = "Time", y = "CO2 Background (ppm)") +
# specify legend values manually
  # specify legend values manually
  scale_color_identity(name = "Background Type",
                     breaks = c("black", "red"),
                     labels = c("Percent", "Minimum"),
                     guide = "legend") +
   # theme
  theme_bw() + 
  theme(legend.position = "bottom")
  
  #TODO Chris:  Legend isn't working correctly
...
variation in background isn't that large in the context of the complete CO$_2$
dataset.


```{r loess smooth of moving minimum}
#---- loess smooth of moving minimum ----

# specify loess model
mdl_pct <- loess(co2_background_pct ~ seq_along(time), data = ts_data, span = 0.1, degree = 1)
mdl_min <- loess(co2_background_min ~ seq_along(time), data = ts_data, span = 0.1, degree = 1)

# add smooted predictions of background to ts_data
ts_data <- ts_data %>% 
...
# compare all background estimates
  ggplot(data = ts_data) + 
    geom_line(aes(x = datetime, y = co2_background_pct), lty = 1, color = "black") + 
    geom_line(aes(x = datetime, y = co2_background_min), lty = 1, color = "red") + 
    geom_line(aes(x = datetime, y = co2_background_pctl), lty = 2, color = "black") + 
    geom_line(aes(x = datetime, y = co2_background_minl), lty = 2, color = "red") + 
    labs(x = "Time", y = "CO2 Background (ppm)") +
...
# specify legend values manually
  ...
  scale_linetype_manual(name = "Background Type",
                        ...
                        labels = c("Unsmoothed", "Smoothed")) +

  # theme
  theme_bw() + 
  theme(legend.position = "bottom")

# TODO: Chris- legends aren't working
...
ggplot(data = ts_data) + 
  ...
  # plot co2 background using the loeess smoothed rolling minimum
  geom_line(aes(x = datetime, y = co2_background_minl), lty = 1) + 
  # plot co2 background using the unsmoothed rolling minimum
...
<!-- Elena, 
Please add to or edit as needed, this was material I wasn't so sure of. In particular, whether or not  -->
...
With background removed from the CO<sub>2</sub> concentrations, we can now use the
adjusted data to identify concentration peaks. Concentration peaks indicate
potential sources of the pollutant. In this case, a CO<sub>2</sub> peak could be a large
...
As the CO<sub>2</sub> map shows, the data we've collected represent concentrations over
...
variability observed in time series plots) processes.
...
concentrations to CO<sub>2</sub> concentrations, potentially helping identify emission
sources in data with both temporal and spatial variability.
